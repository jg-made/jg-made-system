...
function papidb(){
    ...
    PGPASSWORD=$(vault_grep password secret/services/procurement/db) pgcli -h $(consul kv get service/procurement/db/host) -U procurement
}
alias papidb=papidb;

function papivizpdf(){
    vault_check_token || return 1;
    PGPASSWORD=$(vault_grep password secret/services/procurement/db) eralchemy -i "postgresql+psycopg2://procurement@$(consul kv get service/procurement/db/host)" -o ~/Desktop/papivizpdf_$(date +%F_%T).pdf;
}
...
function papipg_dump(){
    ...
    PGPASSWORD=$(vault_grep password secret/services/procurement/db) pg_dump -h $(consul kv get service/procurement/db/host) -U procurement | psql -h localhost -d papi -U procurement
